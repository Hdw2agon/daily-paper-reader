...
tldr: 针对现实场景中图像退化（如模糊、噪声）损害多模态理解的问题，本文提出 CLEAR 框架。该框架通过监督微调建立“先生成后回答”的推理模式，利用潜空间表示桥接实现生成与推理的直接优化，并采用交替 GRPO 强化学习协同优化文本与视觉生成。实验表明，CLEAR 在 MMD-Bench 基准上显著提升了模型对退化图像的鲁棒性，且不损失清晰图像的性能。
selection_source: fresh_fetch
motivation: 现有的统一多模态模型在处理退化图像时，未能有效利用其自身的生成能力来辅助视觉结构的恢复与理解。
method: 通过退化感知微调、潜空间表示桥接以及交替 GRPO 强化学习，构建并优化“先生成后推理”的联合路径。
result: 在 MMD-Bench 的多种退化场景下，CLEAR 显著提高了模型对低质量图像的理解能力，同时保持了在清晰图像上的表现。
conclusion: 研究证明任务驱动的优化能使中间视觉生成状态与感知质量自然对齐，从而有效解锁生成能力在复杂环境下的理解潜力。
---

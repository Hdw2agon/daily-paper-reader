...
## 摘要
可泛化 3D 高斯泼溅（G-3DGS）已成为稀疏视角设置下新视角合成的一种极具前景的方法。然而，现有框架仍受限于像素对齐的高斯估计，这在部分观测或遮挡区域表现不佳，且常导致表面不完整或结构坍塌。为了应对这些挑战，我们提出了 SeeU（Seeing the Unseen），这是一个新颖的 G-3DGS 框架。我们将其核心设计构架为 Semantic-in-Gaussian：高斯空间中基于语义条件的细化。具体而言，我们引入了一个跨视角熵感知（CEA）模块，将多视角语义和几何线索聚合为紧凑的嵌入。这些嵌入引导条件高斯 Transformer 对粗糙高斯进行残差更新，有助于在保持表面一致性的同时，恢复部分观测结构中受约束不足的区域。在多个基准测试上的全面实验表明，SeeU 在保持高效前馈推理的同时，持续提升了渲染质量和结构完整性。特别是在具有挑战性的外推设置下，与最近的 SOTA G-3DGS 方法相比，SeeU 在 PSNR 上实现了平均 2.44 dB 的提升。

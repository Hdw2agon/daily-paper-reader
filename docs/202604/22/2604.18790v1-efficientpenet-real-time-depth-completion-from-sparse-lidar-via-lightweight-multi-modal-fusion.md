...
tldr: 针对现有深度补全模型计算量大、难以在嵌入式设备实时部署的问题，本文提出EfficientPENet。该网络采用轻量化双分支架构，结合现代化的ConvNeXt骨干网络与稀疏不变卷积，并通过卷积空间传播网络（CSPN）进行精细化处理。在KITTI基准测试中，该模型在保持高精度的同时，显著降低了参数量并提升了推理速度，是边缘计算平台上实时深度补全的理想方案。
selection_source: fresh_fetch
motivation: 现有的深度补全方法虽然精度高，但由于模型过于沉重，无法在资源受限的嵌入式硬件上实现实时运行。
method: 采用基于ConvNeXt的轻量化双分支编码器，结合稀疏不变卷积处理深度流，并通过后期融合、多尺度监督及CSPN模块优化预测结果。
result: 在KITTI榜单上达到631.94 mm的RMSE，参数量仅为36.24M，推理速度达48.76 FPS，比BP-Net快23倍。
conclusion: EfficientPENet在精度与效率之间取得了极佳平衡，为自动驾驶等实时机器人系统的3D感知提供了实用的边缘端解决方案。
---

## 摘要
从稀疏激光雷达（LiDAR）测量值和相应的 RGB 图像进行深度补全是机器人系统中实现精确 3D 感知的先决条件。现有方法在标准基准测试中取得了很高的精度，但依赖于沉重的主干架构，这阻碍了在嵌入式硬件上的实时部署。我们提出了 EfficientPENet，这是一种双分支深度补全网络，它使用现代化的 ConvNeXt 主干替换了传统的 ResNet 编码器，为深度流引入了稀疏不变卷积，并通过卷积空间传播网络（CSPN）对预测进行细化。RGB 分支利用了经过 ImageNet 预训练的 ConvNeXt 模块，包含层归一化（Layer Normalization）、7x7 深度可分离卷积和随机深度正则化。来自两个分支的特征通过后期融合进行合并，并通过多尺度深度监督策略进行解码。我们还引入了一种位置感知测试时增强方案，该方案在水平翻转期间校正坐标张量，从而在推理时实现一致的误差降低。在 KITTI 深度补全基准测试中，EfficientPENet 实现了 631.94 mm 的 RMSE，参数量为 36.24M，延迟为 20.51 ms，运行速度为 48.76 FPS。与 BP-Net 相比，这代表参数量减少了 3.7 倍，速度提升了 23 倍，同时保持了具有竞争力的精度。这些结果证明 EfficientPENet 是在 NVIDIA Jetson 等资源受限的边缘平台上实现实时深度补全的实用解决方案。

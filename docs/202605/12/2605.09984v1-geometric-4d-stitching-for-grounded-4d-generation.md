...
## 摘要
最近的 4D 生成方法利用生成模型补全场景级的缺失信息，并将场景重建为基于辐射的表示。然而，这些流水线在生成内容中经常出现几何不一致性，且基于辐射的重建需要昂贵的优化。此外，基于辐射的表示往往将这些几何不一致性吸收进其视图依赖特性中，未能强制执行扎实的几何一致性。为了解决这些问题，我们提出了几何 4D 缝合（Geometric 4D Stitching），这是一个高效的框架，能够显式识别缺失的几何区域，并使用具有几何约束的 4D 缝合块对其进行补充。结果表明，我们的方法在单张 NVIDIA RTX 5090 GPU 上，每次单步场景扩展可在 10 分钟内构建 4D 场景表示，同时提高了几何一致性。此外，我们证明了我们的显式 4D 缝合支持 4D 网格的交互式扩展以及 4D 场景编辑。

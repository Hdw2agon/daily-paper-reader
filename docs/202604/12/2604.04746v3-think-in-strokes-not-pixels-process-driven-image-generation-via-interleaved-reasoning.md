...
tldr: 针对传统图像生成模型缺乏人类绘画般的增量推理过程，本文提出了一种“过程驱动”的图像生成范式。该方法将生成过程分解为文本规划、视觉草绘、文本反思和视觉细化四个交替阶段，通过交替的文本与视觉推理轨迹，使生成过程更具解释性。通过引入密集的逐步监督机制，确保了中间状态的一致性并允许模型自我纠错，在多个基准测试中验证了其有效性。
selection_source: fresh_fetch
motivation: 传统模型通常单步生成图像，缺乏人类绘画中那种基于演化视觉状态的增量规划、反思和细化过程。
method: 提出一种包含文本规划、视觉草绘、文本反思和视觉细化四个阶段的交替推理框架，并结合密集的逐步监督来维持空间语义一致性。
result: 实验证明该方法在多个文本生成图像基准测试中表现优异，使生成过程变得显式、可解释且可直接监督。
conclusion: 过程驱动的交替推理范式能有效模拟人类绘画逻辑，通过中间状态的反馈循环显著提升了图像生成的质量和可控性。
---

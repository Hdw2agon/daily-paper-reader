...
motivation: 稀疏视角下的动态场景重建存在严重的病态问题，且传统COLMAP初始化导致几何信息严重缺失。
method: 通过多视角深度融合提供稠密几何初始化，并利用视频扩散模型生成的修复序列作为伪监督进行迭代优化。
result: 在基准数据集上表现优异，PSNR指标较现有最优方法提升了近2 dB。
conclusion: 该方法有效解决了稀疏视角下4D高斯表征的几何缺失问题，实现了高质量的动态场景合成。
---

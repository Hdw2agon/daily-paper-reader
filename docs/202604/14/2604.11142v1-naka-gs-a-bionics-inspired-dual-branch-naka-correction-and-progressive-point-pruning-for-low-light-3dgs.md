...
score: 7.0
evidence: 光度修复与3D重建
tldr: 针对低光照环境下3D重建面临的能见度低、色彩失真及几何先验受损等挑战，本文提出Naka-GS框架。该方法受仿生学启发，通过双分支Naka校正网络提升图像质量，并利用轻量化点云预处理模块（PPM）进行坐标对齐与渐进式剪枝，优化3D高斯泼溅（3DGS）的初始化。在NTIRE 3DRR挑战赛中，该方法显著提升了低光场景的恢复质量与训练效率。
selection_source: fresh_fetch
motivation: 低光照条件会严重降低图像能见度并引入色彩失真，从而干扰3D重建中的几何先验和优化过程。
method: 提出结合物理先验的双分支Naka校正网络进行图像增强，并配合包含渐进式剪枝的点云预处理模块优化3DGS初始化。
result: 在NTIRE 3DRR挑战赛中表现优异，大幅超越基准方法，实现了更高质量的色彩恢复和更稳定的几何重建。
conclusion: Naka-GS在不增加推理负担的前提下，有效解决了低光3D重建中的光度恢复与几何初始化难题。
---

## 摘要
低光照条件通过降低图像可见度、引入色彩失真以及污染下游优化的几何先验，严重阻碍了 3D 修复与重建。我们提出了 NAKA-GS，这是一个用于低光照 3D 高斯泼溅（3D Gaussian Splatting）的仿生启发式框架，可同时提升光度修复和几何初始化。我们的方法始于一个 Naka 引导的色度校正网络，该网络结合了物理先验低光增强、双分支输入建模、频率解耦校正以及掩码引导优化，以抑制亮区的色度伪影和边缘结构误差。增强后的图像随后被输入到一个前馈多视图重建模型中，以生成稠密的场景先验。为了进一步改进高斯初始化，我们引入了一个轻量级的点预处理模块（PPM），该模块执行坐标对齐、体素池化和距离自适应渐进式剪枝，在保留代表性结构的同时去除噪声和冗余点。在不引入沉重推理开销的情况下，NAKA-GS 提升了低光照 3D 重建的修复质量、训练稳定性和优化效率。所提方法在 NTIRE 3D 修复与重建（3DRR）挑战赛中展示，并以显著优势超过了基准方法。代码可在 https://github.com/RunyuZhu/Naka-GS 获取。

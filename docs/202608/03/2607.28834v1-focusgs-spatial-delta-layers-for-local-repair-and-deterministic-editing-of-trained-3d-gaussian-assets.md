...
title_zh: FocusGS：针对已训练 3D 高斯资产局部修复与确定性编辑的空间增量层
...
## 摘要
3D 高斯泼溅 (3D Gaussian Splatting, 3DGS) 正在从一次性重建演变为可交付、可检查且可维护的视觉资产。现有工作流侧重于全局重建、训练时密度控制或开放式生成编辑，使得已训练资产缺乏精确的局部维护。我们提出了 FocusGS，它将局部修复和确定性编辑统一为复合空间增量。修复是纯加性的特例：其基础操作项为空，仅添加局部高斯基元；确定性编辑使用擦除-插入分解 (EIF) 来结合旧载体擦除与新内容插入。FocusGS 解决了空间梯度饥饿问题：在 93 个评估视角下，局部修复将目标区域的 PSNR 提高了 7.91 dB。在所有 83 次确定性编辑试验中，目标感兴趣区域 (ROI) 均得到改善，试验平均编辑 ROI PSNR 为 21.97 dB，平均增益为 +11.05 dB；在五个公开编辑案例中，FocusGS-EIF 达到了 33.17 dB 的目标掩码 PSNR 和 0.994 的目标增量相关性，而两种文本驱动的基线方法均未能完成预定的更新。FocusGS 提供了一种轻量级、可验证的 3DGS 维护算子。

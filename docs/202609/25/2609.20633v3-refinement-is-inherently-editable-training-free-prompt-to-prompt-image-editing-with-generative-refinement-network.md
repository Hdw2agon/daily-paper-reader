...
date: 2026-09-17
pdf: "https://arxiv.org/pdf/2609.20633v3"
tags: ["query:q4"]
score: 6.0
evidence: 使用生成网络的免训练图像编辑
tldr: RefineEdit 是一种无需训练的 Prompt-to-Prompt 图像编辑框架，利用生成式细化网络（GRN）的特性，通过全局二进制图像编码细化实现编辑。该方法通过位路由、自适应空间冻结和有限位锁定机制，在无需外部掩码或注意力控制的情况下，实现了在保留背景的同时精准执行文本引导的图像编辑，在多项指标上优于现有方法。
selection_source: fresh_fetch
motivation: 现有扩散模型和自回归编辑方法在处理编辑完整性与背景保留时存在局限，且缺乏灵活的修正机制。
method: 通过位路由技术识别可编辑区域，并结合自适应空间冻结与有限位锁定机制，在生成过程中动态调整编辑内容。
result: 在 PIE-Bench 的九类编辑任务中，该方法在背景保留指标（PSNR、LPIPS等）及 CLIP 分数上均达到最优表现。
conclusion: 证明了生成式细化网络在图像编辑任务中的天然优势，为无需训练的精准图像编辑提供了一种高效且鲁棒的新范式。
---

## 摘要
文本引导的图像编辑必须在引入所需更改的同时保留不相关的源内容。在免训练编辑中，扩散模型编辑器通常使用空间控制，但其不准确性可能导致编辑不完整或改变不相关的区域。因果自回归编辑器面临进一步的约束：其固定的解码顺序限制了对早期决策的修正。作为首个探索利用生成式精炼网络（GRN）进行免训练图像编辑的研究，我们观察到其精炼过程本质上非常适合编辑，并为解决上述局限性提供了一种有前景的方法。受此启发，我们引入了 RefineEdit，这是一个基于 GRN 构建的免训练 Prompt-to-Prompt 图像编辑框架。我们的核心思想是通过二进制图像编码的全局精炼，将编辑定位与内容生成相结合，从而允许在图像演化过程中修正编辑证据。具体而言，RefineEdit 将位路由（bit routing）与两种稳定机制相结合：自适应空间冻结和有限位锁定。位路由从中间源状态开始，利用两个分支之间的符号概率差来识别可编辑的位置和位。它引导选定的位进行编辑精炼，同时将其余部分锚定在演化的源轨迹上。自适应空间冻结限制了编辑区域的不必要扩展，而有限位锁定则保持了最近的位激活以支持持续编辑。整个框架无需额外的训练、外部掩码或注意力控制。在 PIE-Bench 的九个编辑类别中，RefineEdit 在 PSNR、LPIPS、MSE 和 SSIM 指标上实现了最佳的背景保留分数，并在所有评估方法中获得了最高的整图和编辑区域 CLIP 分数。代码可在 https://github.com/mura1n/RefineEdit 获取。

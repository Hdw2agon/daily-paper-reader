...
motivation: 现有图像生成3D模型往往缺乏几何约束，难以满足高精度应用需求。
method: 提出一种基于射线一致性观测似然的测试时引导方法，将部分几何观测信息直接融入预训练模型的生成过程。
result: 在不同观测条件下，该方法显著提升了生成模型的几何保真度和视觉质量。
conclusion: 证明了通过显式的测试时引导，无需微调即可有效整合部分观测信息，从而增强预训练3D生成模型的能力。
---

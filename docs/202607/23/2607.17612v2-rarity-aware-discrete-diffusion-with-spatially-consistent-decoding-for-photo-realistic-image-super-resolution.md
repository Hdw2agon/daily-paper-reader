...
title_zh: 用于逼真图像超分辨率的稀有度感知离散扩散与空间一致性解码
...
tldr: 针对超分辨率（SR）任务中连续扩散模型语义表征不直接及自回归模型效率低的问题，本文提出DiMOO-SR框架。该框架采用离散扩散模型，通过逆频率采样（IFS）解决视觉标记的长尾分布问题，并利用空间一致性排序（SCR）优化推理时的结构连贯性。实验证明，DiMOO-SR在保持高感知质量的同时，仅需少量并行解码步骤即可实现高效的真实感图像超分辨率重建。
selection_source: fresh_fetch
motivation: 现有的连续扩散模型难以直接利用多模态模型的标记化范式，而离散扩散模型在SR中面临稀有纹理表征不足和并行解码空间不一致的挑战。
method: 提出DiMOO-SR框架，在训练阶段引入逆频率采样（IFS）关注稀有标记，在推理阶段采用空间一致性排序（SCR）提升局部结构连贯性。
result: 在多个真实世界SR基准测试中，DiMOO-SR以极少的并行解码步数实现了具有竞争力的感知质量。
conclusion: 离散扩散模型在生成式图像超分辨率领域具有巨大潜力，通过解决标记分布和空间一致性问题可实现高效且高质量的重建。
---

## 摘要
连续扩散模型已成为逼真图像超分辨率（SR）的主流范式，但它们通常将重建建模为连续信号级的去噪，并通过外部条件模块引入语义先验。这使得利用现代多模态模型中统一的基于 token 的扩展范式变得不够直接。自回归模型通过将图像建模为离散视觉 token 提供了更原生的语义表示，但其因果解码对于高分辨率重建效率较低。离散扩散通过在视觉 token 上实现非因果的并行预测，提供了一个极具前景的折中方案。然而，由于两个特定任务的挑战，将离散扩散直接应用于 SR 并非易事：(1) 视觉 token 的长尾分布导致稀有但感知关键的纹理表现不足；(2) 空间不一致的并行解码可能会引入孤立的伪影。为了解决这些问题，我们提出了 DiMOO-SR，这是一个用于逼真图像 SR 的稀有度感知多模态离散扩散框架。在训练期间，逆频率采样（IFS）优先处理表现不足但信息丰富的 token。在推理期间，空间一致性排序（SCR）利用局部邻域一致性来优化 token 置信度，从而提高结构连贯性。在广泛使用的真实世界 SR 基准测试上的大量实验表明，DiMOO-SR 仅需少量并行解码步骤即可达到具有竞争力的感知质量，凸显了离散扩散在生成式图像超分辨率方面的潜力。代码将在论文发表后发布。

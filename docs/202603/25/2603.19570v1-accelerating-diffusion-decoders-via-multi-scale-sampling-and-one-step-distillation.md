...
tldr: 针对扩散模型解码器在图像分词中推理速度慢的问题，本文提出了一种两阶段加速框架。该框架首先采用多尺度采样策略，从低分辨率逐步细化至高分辨率以实现理论加速；随后通过知识蒸馏将各尺度的扩散过程简化为单步去噪。该方法在保持高感知保真度的同时，将解码时间缩短了一个数量级，为高效图像分词提供了实用方案。
selection_source: fresh_fetch
motivation: 扩散解码器虽能实现高保真图像重建，但其迭代采样过程带来的高延迟限制了其在实时或大规模应用中的普及。
method: 提出了一种结合多尺度逐步细化采样与单步去噪模型蒸馏的两阶段加速框架。
result: 在几乎不损失图像重建质量的前提下，实现了解码速度的数量级提升。
conclusion: 该研究为构建兼顾效率与表现力的图像分词器提供了有效路径，并为高效视觉生成奠定了基础。
---

## 摘要
图像令牌化在现代生成建模中起着核心作用，它将视觉输入映射为紧凑的表示，作为像素与生成模型之间的中间信号。基于扩散的解码器最近被应用于图像令牌化，以从潜表示中重建具有高感知保真度的图像。与用于下游生成的扩散模型不同，这些解码器致力于忠实重建而非内容生成。然而，它们的迭代采样过程引入了显著的延迟，使其在实时或大规模应用中变得不切实际。在这项工作中，我们引入了一个两阶段加速框架来解决这一效率低下的问题。首先，我们提出了一种多尺度采样策略，解码从粗分辨率开始，并通过在每个阶段将分辨率翻倍来逐步细化输出，与标准的全分辨率采样相比，实现了 $\mathcal{O}(\log n)$ 的理论加速。其次，我们将每个尺度的扩散解码器蒸馏为单步去噪模型，从而在每个尺度的一次前向传递中实现快速且高质量的重建。这些技术共同作用，在输出质量几乎没有下降的情况下，使解码时间减少了一个数量级。我们的方法为构建高效且具有表现力的图像令牌器提供了一条切实可行的途径。我们希望这项工作能为未来在高效视觉令牌化和下游生成领域的研究奠定基础。

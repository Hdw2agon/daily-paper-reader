...
tags: ["query:q1"]
score: 6.0
evidence: 将场景补全与语义分割解耦
tldr: 针对自动驾驶中3D语义占据预测计算开销大和几何结构模糊的问题，本文提出SparseOcc++。该框架通过场景补全场（SCF）将场景补全与语义分割解耦，利用稀疏锚点进行符号距离回归，并结合几何引导传播模块。SparseOcc++在保持精细3D结构的同时，显著提升了预测精度和推理速度，在nuScenes和SemanticKITTI数据集上均达到SOTA水平。
selection_source: fresh_fetch
motivation: 现有的全稀疏表示方法将场景补全与语义预测耦合，导致计算冗余和几何歧义。
method: 提出一种几何感知的稀疏框架，通过场景补全场进行符号距离回归，并利用几何引导传播模块将语义分割限制在验证区域。
result: 在nuScenes上IoU提升2.3点且速度快3.9倍，在SemanticKITTI上比OccFormer快5.9倍。
conclusion: SparseOcc++通过解耦几何与语义，实现了高效且高精度的3D语义占据预测，为自动驾驶感知提供了更优的稀疏表征方案。
---

## 摘要
基于视觉的 3D 语义占用预测对自动驾驶至关重要，然而密集型体素表示在大量空白空间上浪费了计算资源，而 BEV 和 TPV 投影则损害了细粒度的 3D 结构。全稀疏表示提供了一个极具吸引力的替代方案，但包括 SparseOcc 在内的现有方法通过将高维特征不加区分地传播到空白区域并应用逐体素分类，将场景补全与语义预测纠缠在一起。这导致了过度的激活、计算开销和几何歧义。我们提出了 SparseOcc++，这是一个几何感知的稀疏框架，它显式地将场景补全与语义分割解耦。SparseOcc++ 通过场景补全场 (SCF) 将补全任务重新表述为稀疏锚点体素上的符号距离回归。为了稳健地建模复杂的室外几何结构，它结合了正交分解与离散化距离学习。随后，一个几何引导的传播模块将 SCF 转换为完整的体积场景，并将语义分割限制在经过几何验证的区域。实验确立了新的领域领先水平：在 nuScenes 数据集上，SparseOcc++ 的 IoU 提升了 2.3 个百分点，速度比 SparseOcc 快 3.9 倍；在 SemanticKITTI 数据集上，其速度比 OccFormer 快 5.9 倍。

...
title_zh: 用于写实图像超分辨率的稀有度感知空间一致性解码离散扩散模型
...
tldr: 本研究针对图像超分辨率（SR）任务，提出了一种名为DiMOO-SR的稀有度感知多模态离散扩散框架。传统连续扩散模型缺乏原生语义表征，而自回归模型解码效率低下。DiMOO-SR利用离散扩散的并行预测优势，通过逆频率采样（IFS）解决视觉标记分布不均导致的稀有纹理缺失问题，并引入空间一致性排序（SCR）消除并行解码中的孤立伪影。实验证明，该方法在保持高感知质量的同时显著提升了推理效率，为生成式超分辨率提供了新思路。
selection_source: fresh_fetch
motivation: 旨在解决连续扩散模型语义表征不直接以及自回归模型在图像重建中解码效率低下的问题。
method: 提出DiMOO-SR框架，在训练阶段采用逆频率采样强化稀有纹理学习，在推理阶段利用空间一致性排序优化结构连贯性。
result: 在多个真实世界超分辨率基准测试中，DiMOO-SR仅需少量并行解码步骤即可实现极具竞争力的感知质量。
conclusion: 离散扩散模型结合稀有度感知与空间一致性策略，是实现高效、高质量生成式图像超分辨率的有效途径。
---

## 摘要
连续扩散模型已成为写实图像超分辨率（SR）的主流范式，但它们通常将重建建模为连续信号级的去噪，并通过外部条件模块引入语义先验。这使得利用现代多模态模型中统一的基于 token 的缩放范式变得不够直接。自回归模型通过将图像建模为离散视觉 token 提供了更原生的语义表示，但其因果解码对于高分辨率重建效率较低。离散扩散模型通过在视觉 token 上实现非因果、并行的预测，提供了一个极具前景的折中方案。然而，由于两个特定任务的挑战，将离散扩散直接应用于 SR 并非易事：(1) 视觉 token 的长尾分布，导致稀有但感知关键的纹理表示不足；(2) 空间不一致的并行解码，可能会引入孤立的伪影。为了解决这些问题，我们提出了 DiMOO-SR，这是一个用于写实图像 SR 的稀有度感知多模态离散扩散框架。在训练期间，逆频率采样（IFS）优先处理表示不足但信息丰富的 token。在推理期间，空间一致性排序（SCR）利用局部邻域一致性来细化 token 置信度，从而提高结构连贯性。在广泛使用的真实世界 SR 基准测试上的大量实验表明，DiMOO-SR 仅需少量并行解码步骤即可实现具有竞争力的感知质量，突显了离散扩散在生成式图像超分辨率方面的潜力。代码将在论文发表后发布。

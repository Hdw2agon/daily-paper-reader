...
tags: ["query:q1"]
score: 7.0
evidence: 在3D重建视图中填充缺失区域
tldr: Leveling3D 针对前馈 3D 重建（如 3DGS）在推断视角下存在伪影和缺失区域的问题，提出了一种结合几何感知生成的增强框架。通过引入轻量级的几何平滑适配器将扩散模型的生成能力与几何先验对齐，并结合调色板过滤和掩码细化策略，实现了高质量的视角修复与重建。该方法在多项任务上达到 SOTA 水平，显著提升了 3D 重建的完整性与一致性。
selection_source: fresh_fetch
motivation: 现有的前馈 3D 重建方法在处理外推视角时常出现伪影且缺乏几何一致性，导致生成结果不完整。
method: 提出几何感知平滑适配器以对齐扩散模型与几何先验，并结合调色板过滤训练策略和测试时掩码细化来优化生成质量。
result: 在公开数据集的新视角合成和深度估计任务上均取得了最先进（SOTA）的性能表现。
conclusion: Leveling3D 通过整合几何一致性生成与前馈 3DGS，有效解决了 3D 重建中的伪影问题，提升了整体重建质量。
---

...
tldr: 本研究针对像素空间扩散模型优化困难的问题，指出其根源在于高噪声下强制预测全图细节导致的“尺度-时间失配”。为此，提出观测算子扩散（Observation Operator Diffusion）框架，通过高斯-兰乔斯算子构建从粗到精的动态监督轨迹，使模型学习过程与图像结构的自然恢复顺序一致。此外，引入GL-CoDA解码器增强特征细化。该方法在ImageNet-256上实现1.52的FID，显著提升了收敛速度和生成质量。
selection_source: fresh_fetch
motivation: 现有像素空间扩散模型在训练初期强制预测全图细节，导致梯度信噪比低且优化困难。
method: 提出观测算子扩散框架，利用高斯-兰乔斯算子构建随时间从粗糙到精细演化的监督轨迹，并配合GL-CoDA解码器进行特征细化。
result: 该方法显著加快了收敛速度，并在ImageNet-256数据集上达到了1.52的FID，刷新了生成质量记录。
conclusion: 通过将监督轨迹与图像结构的内在恢复顺序对齐，有效解决了扩散模型中的尺度-时间失配问题，提升了训练效率。
---

## 摘要
像素空间扩散模型直接对图像分布建模，但其优化仍然具有挑战性。近期的方法通过目标重参数化缓解了这一挑战，但在整个去噪过程中仍依赖于固定的清晰图像目标。通过经验分析，我们发现了一种尺度-时间不匹配现象：随着噪声降低，图像结构从粗糙到精细变得可预测，而现有模型即使在高噪声下也被迫预测完整图像，导致低信噪比（SNR）梯度，从而阻碍了优化。为了解决这种不匹配，我们提出了观测算子扩散（Observation Operator Diffusion），这是一个统一的框架，将监督轨迹和特征细化与图像结构的内在恢复顺序对齐。具体而言，我们将标准流路径上的固定全图监督替换为随时间索引的观测轨迹，该轨迹在去噪过程中从粗糙结构演变为完整图像。该轨迹由一系列不同观测尺度的 Gaussian-Lanczos 算子实例化，从而产生路径一致的训练目标。我们还引入了 GL-CoDA，这是一种在解码阶段注入特定尺度 Gaussian-Lanczos 观测值的解码器，用于实现从粗到精的特征细化。大量实验表明，所提方法在持续提高生成质量的同时，收敛速度显著加快，在 ImageNet-256 上实现了 1.52 的 FID。

...
tags: ["query:q4"]
score: 6.0
evidence: 用于视觉证据的生成式匿名化模块
tldr: 针对多模态检索增强生成（MRAG）中检索图像包含敏感人脸隐私的问题，本文提出了一种身份解耦的匿名化框架。该框架通过变分编码器将人脸分解为身份和属性特征，利用拒绝采样生成虚拟身份，并结合潜在扩散模型合成匿名图像。在保护隐私的同时，该方法保留了下游推理所需的视觉线索，实现了高效且具备隐私保障的视觉证据处理。
...
result: 该方法在确保身份相似度降至冒充阈值以下的同时，有效保留了非身份相关的视觉属性，并支持低延迟部署。
conclusion: 本研究为多模态检索增强生成提供了一种兼顾隐私保护与信息保留的生成式匿名化方案。
---

## 摘要
多模态检索增强生成（MRAG）系统从大规模图像语料库中检索视觉证据，以支撑大型多模态模型的响应，但检索到的图像通常包含人脸，其身份信息属于敏感个人信息。现有的匿名化技术要么破坏了下游推理所依赖的非身份视觉线索，要么未能提供原则性的隐私保证。我们提出了身份解耦 MRAG（Identity-Decoupled MRAG），这是一个在检索和生成之间插入生成式匿名化模块的框架。我们的方法由三个组件组成：(i) 一个解耦变分编码器，将每张人脸分解为身份代码和空间结构属性代码，并通过互信息惩罚和基于梯度的独立性项进行正则化；(ii) 一个流形感知拒绝采样器，用合成代码替换身份代码，确保其既与原始代码不同又具有真实感；(iii) 一个条件潜扩散生成器，根据替换的身份和保留的属性合成匿名化人脸，并将其蒸馏为潜一致性模型以实现低延迟部署。隐私保护通过人脸识别模型的多预言机集成来实现，并采用基于铰链（hinge-based）的损失函数，一旦身份相似度降至冒充者阈值以下，即停止优化。

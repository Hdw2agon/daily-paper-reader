...
tldr: 本文提出了一种过程驱动的图像生成范式，旨在模拟人类绘画时的增量推理过程。该方法将图像生成分解为文本规划、视觉草绘、文本反思和视觉细化四个交替阶段，通过交替的文本与视觉推理轨迹，使生成过程更具解释性。研究引入了密集的逐步监督机制，以解决中间状态的歧义性，确保了视觉一致性并增强了模型识别与修正错误的能力，在多个基准测试中展现了卓越性能。
selection_source: fresh_fetch
motivation: 传统图像生成模型通常采用单步生成，缺乏人类绘画中那种基于演化视觉状态进行增量规划、反思和细化的多步推理能力。
method: 提出一种交替推理框架，通过文本规划、视觉草绘、文本反思和视觉细化四个阶段，结合密集的逐步监督来维持空间语义一致性。
result: 实验表明，该方法使生成过程显式且可解释，并在多个文本到图像生成基准测试中取得了显著的性能提升。
conclusion: 过程驱动的生成范式通过模拟人类绘画逻辑，有效提升了多模态模型在复杂生成任务中的可控性、可解释性和图像质量。
---

...
## 摘要
3D 资产的可控局部编辑需要精确的目标定位和适当的视觉引导。然而，现有方法缺乏一种简单且准确的获取 3D 掩码的方式，并且在实现预期编辑的同时，难以忠实地保留非目标区域的结构和外观。为了应对这些挑战，我们提出了 EditFlow3D，这是一个无需训练的局部 3D 编辑框架。给定源资产和编辑指令，一个由视觉语言模型（VLM）驱动的工作流会解释编辑意图，并自动构建视觉引导图像和精细的 3D 编辑掩码，从而在预训练 3D 生成模型的原生表示空间中实现局部编辑。具体而言，掩码引导的微分流将编辑集中在目标区域，而逐步轨迹保持在不直接替换中间特征的情况下，维持了非目标区域与源资产之间的一致性。由于现有的 Edit3D-Bench 仅涵盖有限的局部编辑类别，我们进一步引入了 EditFlow-Bench 作为补充基准，涵盖了更广泛的结构和外观编辑，并在两个基准上对 EditFlow3D 进行了评估。定量结果、定性比较和用户研究表明，与现有的 3D 编辑方法相比，EditFlow3D 实现了更准确的目标区域编辑，并能更好地保留非目标区域。

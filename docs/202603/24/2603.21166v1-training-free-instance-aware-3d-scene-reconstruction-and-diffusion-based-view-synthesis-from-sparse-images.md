...
tldr: 该研究提出了一种无需训练的室内场景重建与渲染系统，仅需少量无位姿RGB图像即可实现。系统通过基于扭曲的异常移除策略构建鲁棒点云，利用2D到3D实例提升机制实现实例感知，并结合3D感知扩散模型进行视图合成。该方法不仅能处理稀疏输入下的几何缺失，还支持无需重新训练的对象级场景编辑，为高效、可编辑的3D内容生成提供了新方向。
selection_source: fresh_fetch
motivation: 传统辐射场方法依赖密集视图和逐场景优化，难以在稀疏输入下实现高效且可编辑的场景重建。
method: 整合了基于扭曲的异常点云过滤、2D到3D实例掩码传播以及结合扩散模型的视图细化渲染技术。
result: 在稀疏输入条件下实现了高保真渲染，并支持通过修改点云直接进行一致性的对象级场景编辑。
conclusion: 该系统证明了无需场景特定优化即可实现高效、实例感知且可编辑的3D场景重建与合成。
---

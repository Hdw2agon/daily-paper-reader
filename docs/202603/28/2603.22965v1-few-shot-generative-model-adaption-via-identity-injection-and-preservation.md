...
tags: ["query:q4"]
score: 6.0
evidence: 生成模型适配与图像合成
tldr: 针对少样本生成模型微调中容易丢失源域身份信息并导致模式崩坏的问题，本文提出了身份注入与保留（I2P）框架。该框架通过身份注入模块将源域知识整合至目标域潜空间，并结合包含风格内容解耦的身份替换模块与一致性约束，确保生成图像在迁移至目标域的同时保留源域的关键身份特征。实验表明，该方法在多个基准数据集上显著提升了生成质量，优于现有最先进技术。
selection_source: fresh_fetch
motivation: 现有的少样本生成适配方法在微调过程中往往会遗忘源域的身份知识，从而降低了目标域生成图像的质量。
method: 提出I2P框架，利用身份注入模块整合源域知识，并通过身份替换模块及一致性对齐约束来增强身份信息的保留。
result: 在多个公开数据集和五项评估指标上的定量及定性实验均证明，该方法相比现有SOTA方法取得了实质性的改进。
conclusion: 通过有效地注入和保留源域身份信息，可以显著提升少样本场景下生成模型的适配效果和图像保真度。
---

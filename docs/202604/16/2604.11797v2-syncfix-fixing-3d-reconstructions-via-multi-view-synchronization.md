...
tldr: SyncFix 针对 3D 重建中常见的语义和几何不一致问题，提出了一种基于扩散模型的跨视图一致性增强框架。该方法将修复过程建模为联合潜空间桥接匹配问题，通过在去噪轨迹中同步多个视图的失真与清晰表示，实现了高效的场景优化。SyncFix 仅需图像对训练即可泛化至任意视图，显著提升了重建质量，在无参考或稀疏参考场景下均优于现有技术。
selection_source: fresh_fetch
motivation: 现有的 3D 重建方法常面临跨视图语义和几何不一致的挑战，导致重建结果存在扭曲或伪影。
method: 将修复任务表述为联合潜空间桥接匹配问题，通过在扩散模型去噪过程中学习多视图联合条件来强制执行一致性。
result: 实验表明 SyncFix 在多项指标上超越了现有基线，且重建质量随视图数量增加而提升，支持无参考或稀疏参考修复。
conclusion: SyncFix 提供了一种灵活且强大的多视图同步方案，有效解决了 3D 重建中的不一致性问题，具有良好的泛化性。
---

## 摘要
我们提出了 SyncFix，这是一个在基于扩散的重建场景精细化过程中强制执行跨视图一致性的框架。SyncFix 将精细化建模为一个联合潜空间桥接匹配问题，通过同步多个视图中的失真表示与清晰表示，来修复语义和几何上的不一致。这意味着 SyncFix 学习了多个视图上的联合条件分布，从而在整个去噪轨迹中强制保持一致性。我们的训练仅在图像对上进行，但在推理过程中可以自然地泛化到任意数量的视图。此外，重建质量随视图数量的增加而提高，但在视图数量较多时收益会递减。定性和定量结果表明，即使在缺乏清晰参考图像的情况下，SyncFix 也能稳定地生成高质量的重建结果，并超越了目前最先进的基准方法。当存在稀疏参考时，SyncFix 能达到更高的保真度。

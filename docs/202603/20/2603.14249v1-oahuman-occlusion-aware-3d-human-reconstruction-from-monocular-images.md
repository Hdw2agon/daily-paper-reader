...
tags: ["query:q1"]
score: 7.0
evidence: 遮挡感知重建与纹理合成
tldr: 针对单目图像中由于遮挡导致的3D人体重建不完整和真实感差的问题，本文提出OAHuman框架。该框架通过解耦几何重建与纹理合成，解决了遮挡区域的几何-纹理交叉污染问题。在遮挡区域强化几何感知，并仅从可见区域学习纹理，从而在复杂遮挡环境下实现了高保真、结构完整的3D人体模型重建，显著提升了重建的鲁棒性。
selection_source: fresh_fetch
motivation: 现有方法在处理遮挡时因几何与纹理建模纠缠，导致遮挡区域出现几何缺失和纹理错误。
method: 提出解耦感知范式，显式分离几何重建与纹理合成，在遮挡处强化几何感知并仅从可见区域提取纹理。
result: 在遮挡丰富的基准测试中，该方法在结构完整性、表面细节和纹理真实感方面均达到领先水平。
conclusion: 通过解耦策略有效解决了遮挡下的几何-纹理污染问题，为现实场景下的单目3D人体重建提供了稳健的解决方案。
---

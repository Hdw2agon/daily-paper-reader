...
method: 基于流匹配ODE推导出的区间去噪器框架，通过隔离纯时间导数并结合残差裁剪与时间采样课程学习，实现精确的像素级回归。
result: 在ImageNet 256x256任务中，模型在单步和两步生成下分别取得了4.55和3.98的FID评分。
conclusion: 该研究证明了通过严谨的理论推导和优化策略，可以在像素空间实现高效且高质量的少步生成式建模。
---

## 摘要
现代扩散和基于流的模型正日益转向少步、无潜空间的生成，以绕过多步采样的计算开销和外部自编码器的重建瓶颈。我们提出了区间去噪器（Interval Denoiser），这是一个用于无潜空间生成的理论严谨的框架。该框架直接源自流匹配（flow matching）常微分方程（ODE），为中间轨迹状态建立了精确的解析映射。与之前的公式不同，我们的预测被证明在任何时间间隔内都位于低维流形上，使得直接在像素上运行的网络回归变得可行。此外，通过避免经验性的代数替换，我们的公式正确地分离了纯时间导数，以防止有偏的梯度评估并确保精确的一阶优化。通过分析这一目标函数，我们为框架配备了残差裁剪（residual clipping）和时间采样课程学习（time-sampling curriculum），从而实现了有效的长间隔训练并提升了少步生成的性能。在 ImageNet 256x256 上从头开始训练，我们的模型在不使用感知损失的情况下，单步（1-NFE）达到 4.55 的 FID，两步（2-NFE）达到 3.98 的 FID。

...
## 速览
**TLDR**：本研究针对逆光和低光照图像中存在的曝光不平衡及全局欠曝光问题，提出了一种名为UBLLIE的统一无监督增强框架。该框架利用CLIP引导的提示学习进行语义监督，并结合带有空洞空间金字塔池化（ASPP）的对称残差U-Net架构，以捕捉多尺度上下文信息。通过迭代提示优化机制，UBLLIE在多个数据集上展现了优于现有SOTA方法的保真度和感知质量，为复杂光照环境下的图像增强提供了鲁棒的解决方案。 \
**Motivation**：旨在解决逆光和低光照图像中严重的曝光不平衡及全局欠曝光对视觉感知和下游任务带来的挑战。 \
**Method**：提出一种基于CLIP引导提示学习的无监督框架，采用集成ASPP模块的对称残差U-Net捕捉多尺度上下文信息。 \
**Result**：在BAID、LOL等多个配对和非配对数据集上的实验表明，该方法在保真度、感知质量和泛化能力上均优于现有SOTA方法。 \
**Conclusion**：该框架为现实世界中多样化光照条件的图像增强提供了一种鲁棒且可扩展的无监督解决方案。

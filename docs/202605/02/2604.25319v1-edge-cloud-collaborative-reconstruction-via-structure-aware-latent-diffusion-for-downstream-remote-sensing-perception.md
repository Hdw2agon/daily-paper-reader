...
tldr: 针对高分辨率遥感数据传输带宽受限导致下游感知任务性能下降的问题，本文提出了一种边云协作的结构感知潜在扩散框架（SALD）。该框架在边缘端提取轻量化结构先验，在云端利用结构门控大卷积核和语义引导引擎进行超分辨率重建，有效抑制了生成式模型的幻觉，在极低带宽下显著提升了遥感图像的感知质量和下游检测任务的准确性。
selection_source: fresh_fetch
motivation: 极高压缩比会导致遥感图像关键结构细节丢失，而现有超分方法存在纹理过度平滑或生成结构幻觉的问题，影响下游机器感知。
method: 提出SALD框架，在边缘端传输解耦的低频负载与结构先验，在云端通过结构门控大卷积核和语义引导引擎引导扩散模型重建。
result: 在MSCM和UCMerced数据集上的实验表明，该方法在极低带宽下实现了优异的感知质量，并显著提升了场景分类和小目标检测的性能。
conclusion: 边云协作的结构感知扩散模型能有效平衡传输带宽与重建质量，为受限环境下的遥感图像感知提供了高效的解决方案。
---

## 摘要
高分辨率遥感数据的指数级增长面临着星地传输的严重瓶颈。有限的下行带宽迫使采用极高倍率的压缩，这不可逆地破坏了对目标检测等下游机器感知任务至关重要的高频结构细节。虽然目前的超分辨率技术试图恢复这些细节，但基于回归的方法往往产生过度平滑的纹理，而生成式扩散模型则经常引入结构性幻觉，从而误导检测系统。为了解决这一权衡问题，我们提出了结构感知潜扩散（SALD）框架，这是一种非对称的边云协同超分辨率（SR）系统。在资源受限的边缘端，该系统将图像解耦为高度压缩的低频有效载荷和轻量级的软结构先验。传输这种解耦后的表示可以最大限度地减少带宽消耗。在强大的云端，我们在扩散骨干网络中引入了结构门控大卷积核（SGLK）模块和语义引导引擎（SGE）。这些模块利用传输的结构先验来门控大核卷积，在有效捕捉航空场景固有的长程依赖关系的同时，主动抑制生成幻觉。在 MSCM 和 UCMerced 数据集上的大量实验表明，即使在极端的带宽限制下，SALD 也能获得卓越的感知质量（LPIPS），并显著提升场景分类和小目标检测的下游性能。

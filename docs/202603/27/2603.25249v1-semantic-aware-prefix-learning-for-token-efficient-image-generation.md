...
title_zh: 用于 Token 高效图像生成的语义感知前缀学习
...
## 摘要
视觉分词器通过连接高维图像和可处理的生成建模，在潜空间图像生成中发挥着核心作用。然而，大多数现有的分词器仍采用以重建为主的目标进行训练，这往往导致潜表示与高层语义的关联较弱。最近的方法虽然改进了语义对齐，但通常将语义信号视为辅助正则化，而非使其在表示学习中具有功能上的必要性。我们提出了 SMAP，一种语义感知的前缀分词器，它将类别级语义条件注入到基于查询的一维分词框架中。为了使语义在训练过程中不可或缺，SMAP 引入了一种尾部令牌丢弃策略，该策略迫使语义条件和早期潜前缀在逐渐减少的令牌预算下承担更大的责任。为了验证所得潜空间对生成（而非仅对重建）的有效性，我们进一步引入了 CARD，一种混合因果自回归-扩散生成器。在 ImageNet 上的广泛实验表明，SMAP 在离散和连续分词设置下均能一致地提高重建质量，且其基于语义的潜空间在紧凑的令牌预算下展现出强大的下游生成性能。
